...
class MLP(nn.Module):  # All models should inherit from nn.Module
    ...
    def __init__(
        self, num_input, hidden1_size, num_classes
    ):  # Define our ANN structures here
        super(MLP, self).__init__()
        # nn.Linear(in_features, out_features, bias): y = w^Tx + bias
        self.hidden1 = nn.Linear(
            num_input, hidden1_size
        )  # connection between input and hidden layer1
...
class CNN(nn.Module):
    ...
    def __init__(self, dropout_pr, num_hidden, num_classes):
        ...
        self.conv1 = nn.Conv2d(
            1, 10, kernel_size=5
        )  # K=D=10, output_size W=(28-5)/1+1=24 (24x24), (default S=1)
        self.pool1 = nn.MaxPool2d(
            4, 4
        )  # W = (24-4)/4+1=6 (6x6), S=4 (pool dimension) since no overlapping regions
        self.dropout_conv1 = nn.Dropout2d(
            dropout_pr
        )  # to avoid overfitting by dropping some nodes
        # + You can add more convolutional and pooling layers
        # Fully connected layer after convolutional and pooling layers
        self.num_flatten_nodes = (
            10 * 6 * 6
        )  # Flatten nodes from 10 channels and 6*6 pool_size = 10*6*6=360
        self.fc1 = nn.Linear(self.num_flatten_nodes, num_hidden)
        # + You can add more hidden layers here if necessary
        self.out = nn.Linear(
            num_hidden, num_classes
        )  # the output nodes are 10 classes (10 digits)
...
def show_some_digit_images(images):
    ...
    for i in range(0, 10):
        plt.subplot(
            2, 5, i + 1
        )  # Display each image at i+1 location in 2 rows and 5 columns (total 2*5=10 images)
        plt.imshow(
            images[i][0], cmap="Oranges"
        )  # show ith image from image matrices by color map='Oranges'
    plt.show()
...
def train_ANN_model(
    ...
    if device.type == "cuda" and CUDA_enabled:
        ...
        ANN_model = ANN_model.to(
            device=device
        )  # sending to whaever device (for GPU acceleration)
    ...
    if torch.backends.mps.is_available() and torch.backends.mps.is_built():
        print("...Modeling using Apple Silicon...")
        ...
        for batch_cnt, (images, labels) in enumerate(training_data):
            ...
            if is_MLP:
                ...
                images = images.reshape(
                    -1, 784
                )  # or images.view(-1, 784) or torch.flatten(images, start_dim=1)

            if device.type == "mps" or (device.type == "cuda" and CUDA_enabled):
            ...
            if (batch_cnt + 1) % mini_batch_size == 0:
                print(
                    f"Epoch={epoch_cnt+1}/{num_epochs}, batch={batch_cnt+1}/{num_train_batches}, loss={loss.item()}"
                )
    return train_losses
...
if torch.cuda.is_available():
    ...
    print("NumPy took: ", benchmark_cpu(d), "seconds")
    ...
    C = torch.mm(A, B)
    print("Torch.CUDA took: ", (time.perf_counter() - start_time), "seconds\n")
    # """
    ...
    C = torch.mm(A, B)
    print("NumPy took: ", benchmark_cpu(d), "seconds")
    print("MPS took: ", (time.perf_counter() - start_time), "seconds\n")

else:
    print(
        "CUDA is NOT supported in your machine. Only CPU will be used for computation."
    )
# exit()
...
test_dataset = datasets.MNIST(
    root="./data", train=False, transform=transforms, download=False
)
...
images, labels = next(
    iterable_batches
)  # If you can call next() again, you get the next batch until no more batch left
...
print("> Input digits:")
print(labels)
predicted_digits = test_ANN_model(
    device, CUDA_enabled, is_MLP, MLP_model, test_dataloader
)
...
predicted_digits = test_ANN_model(
    device, CUDA_enabled, is_MLP, CNN_model, test_dataloader
)
...
loaded_MLP_optimizer = MLP_optimizer.load_state_dict(
    torch.load("MLP_optimizer_state_dict.pt")
)
